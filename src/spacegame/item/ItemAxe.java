...
public final class ItemAxe extends Item {
    ...
    public void onRightClick(int x, int y, int z, World world, EntityPlayer player){
        if(world.getBlockID(x,y,z) >= Block.oakLogFullSizeNormal.ID && world.getBlockID(x,y,z) <= Block.oakLogSize1EastWest.ID){
            Random rand = new Random();
            world.setBlockWithNotify(x,y,z, Block.air.ID);
            EntityItem[] sticks = new EntityItem[BlockLog.sizeOfLog(world.getBlockID(x,y,z)) / 2];
