...
public final class GuiMainMenu extends Gui {
    ...
    public void drawGui() {
        ...
        if(this.subMenu){
            ...
            fontRenderer.drawString("Changed the unlit torch item texture to match the block", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("North facing unlit torches will no longer drop mud when broken", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Unlit torch items can now be placed as blocks", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Setting player spawnpoint at a campfire now functions", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Punching animation no longer plays when in a GUI", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Grass no longer stretches too far from wind effect", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Creating a campfire no longer causes a crash", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Campfires when fully created start with 4 firewood in the fuel slot", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Sticks, stones, and clay can no longer be placed in a floating state", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Food no longer gets consumed when opening a chest", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Sticks, stones, clay and seeds no longer have a collision box", x, y ,depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Hitting the escape key in the pause menu will now unpause the game", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Chunks no longer flicker when moving around", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Block faces no longer fail to render after breaking/placing a block", x, y , depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Fully solid chunks with exposed faces should now render", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Tall grass growth is now capped per chunk", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Shifting near blocks with a smaller collision box no longer causes the player to jump up suddenly", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Fall damage calculation now uses the Y value of the block below the player instead of the player's Y value", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Newly spawning clouds no longer affect calculations for determining if it should be raining", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Rain particles now appear on water", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Rain quads no longer overflow", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Rain fog effect now works properly on dropped items, entities and shadows", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Celestial objects no longer move position when moving through chunks", x, y, depth, yellow, 50, 255);
            y -= 30;
            fontRenderer.drawString("Speed is now reduced correctly when jumping out of water", x, y, depth, yellow, 50, 255);
            fontRenderer.toggleItalics();
