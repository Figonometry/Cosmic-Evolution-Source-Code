...
public final class GuiMainMenu extends Gui {
    ...
    public void drawGui() {
        ...
        if(this.subMenu){
            ...
            fontRenderer.drawString("WORLDS FROM PREVIOUS VERSIONS ARE INCOMPATIBLE", x, y, depth, red, 75);
            y -= 30;
            fontRenderer.drawString("Water now is now lowered correctly when waves are off", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed being able to light fires when there are less than 4 logs", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Tools now take damage when hitting deer", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed player inventory slots not being reset upon death", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Added a UI for chests", x, y, depth, green, 50);
            y -= 30;
            fontRenderer.drawString("Added chest saving/loading to the world", x, y, depth, green, 50);
            y -= 30;
            fontRenderer.drawString("Fixed many right click interactions being executed every tick when the mouse is held down", x, y, depth, yellow, 50);
            y -=30;
            fontRenderer.drawString("Added special icons for exclusive inventory slots", x, y, depth, green, 50);
            y -= 30;
            fontRenderer.drawString("Fixed being able to bypass the stack limit of an item by right clicking on it with the same item", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed normals not working on deer when shadows are disabled", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed normals on sticks and rocks", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed incorrect number of items being dropped when removing straw basket from inventory", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Chest collision boxes are now 0.5 blocks tall", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("The player now snaps up on bounding boxes that are less than 0.5 blocks tall", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("Fixed hotbar not properly looping when scrolling", x, y, depth, yellow, 50);
            y -= 30;
            fontRenderer.drawString("The selected hotbar slot is now darker", x, y, depth, yellow, 50);
